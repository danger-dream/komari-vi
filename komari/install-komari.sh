...
# Build download URL for latest or specified version
build_download_url() {
    local file_name="$1"
    ...
    if [ -z "$version_input" ]; then
        ...
        if [ -z "$latest_tag" ]; then
            log_error "无法获取最新的 Komari 版本号，请手动输入版本号。"
            exit 1
        fi
        echo "${RELEASE_BASE}/download/${latest_tag}/${file_name}"
        return
    fi
    local version_tag="$version_input"
    if [[ "$version_tag" != komari-v* ]]; then
        version_tag="komari-v${version_tag#v}"
    fi
    echo "${RELEASE_BASE}/download/${version_tag}/${file_name}"
}

# Fetch latest komari tag from GitHub releases
fetch_latest_tag() {
...
install_binary() {
    ...
    read -p "请输入要安装的版本号 (留空为最新): " input_version
