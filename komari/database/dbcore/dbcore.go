...
func migrateSPPingLatencyColumns(db *gorm.DB) {
	if db == nil || !db.Migrator().HasTable(&models.SPPingRecord{}) {
		return
	}
	cols, err := db.Migrator().ColumnTypes(&models.SPPingRecord{})
	if err != nil {
		log.Printf("failed to inspect sp_ping_records columns: %v", err)
		return
	}
	...
	for _, col := range cols {
		...
		if strings.Contains(dbType, "int") {
			needChange = append(needChange, col.Name())
		}
	}
	if len(needChange) == 0 {
		return
	...
	case "mysql":
		for _, col := range needChange {
			sql := fmt.Sprintf("ALTER TABLE sp_ping_records MODIFY COLUMN %s DOUBLE NOT NULL DEFAULT -1", col)
			if err := db.Exec(sql).Error; err != nil {
				log.Printf("failed to alter sp_ping_records.%s to double: %v", col, err)
			}
		}
	default:
		log.Printf("[sp-ping] latency columns detected as integer on %s, skip alter; SQLite will still store floats on INTEGER columns", db.Dialector.Name())
	}
}
...
func GetDBInstance() *gorm.DB {
	once.Do(func() {
		...
		err = instance.AutoMigrate(
			...
			&models.Client{},
			&models.Credential{},
			&models.InstallScript{},
			&models.Record{},
			&models.GPURecord{},
			&models.LgAuthorization{},
			&models.LgToolSetting{},
			&models.Config{},
			&models.Log{},
			&models.Clipboard{},
