...
type StartRealmRequest struct {
	...
	Config             string            `json:"config"` // TOML 配置
	...
	PriorityConfigs    map[string]string `json:"priority_configs,omitempty"`     // relayNodeID -> entry 配置
	PriorityRelays     []RelayNode       `json:"priority_relays,omitempty"`      // 仅入口节点需要
	ActiveRelayNodeID  string            `json:"active_relay_node_id,omitempty"` // 当前活跃中继
	PriorityListenPort int               `json:"priority_listen_port,omitempty"` // 入口监听端口（冗余传递）
}
