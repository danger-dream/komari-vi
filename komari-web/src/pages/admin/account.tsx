...
import { Badge, Button, Dialog, Flex, Skeleton, TextField } from '@radix-ui/themes'
		...
		try {
			...
			if (ssoInfo?.isBound) {
				...
				if (response.ok) {
					...
					toast.error(
						t('account_settings.unbind_sso_failed', {
		...
		<Flex gap="4" direction="column" align="start">
			<Flex gap="4" direction="row" className="p-4" wrap="wrap">
				<Flex gap="2" direction="column" className="w-full">
					...
					<label className="text-lg">{t('account.greeting', { username: account?.username })}</label>
					<form className="flex gap-2 flex-col" onSubmit={handleSubmitUsernameChange}>
						<label className="font-bold" htmlFor="username">
							{t('account.change_username_title')}
						</label>

						<TextField.Root className="max-w-128" id="username" name="username" defaultValue={account?.username}></TextField.Root>
						<div>
					...
					<form onSubmit={changePassword} className="flex flex-col gap-2">
						...
						<TextField.Root className="max-w-128" id="password" name="password" type="password"></TextField.Root>
						<label htmlFor="password_repeat">{t('account.new_password_repeat')}</label>
						<TextField.Root className="max-w-128" id="password_repeat" name="password_repeat" type="password"></TextField.Root>
						<div>
				...
				<Flex direction="column" className="gap-2">
					...
					{account?.['2fa_enabled'] ? <TwoFactorEnabled /> : <TwoFactorDisabled></TwoFactorDisabled>}
					<label className="font-bold text-2xl mt-2">{t('settings.sso.title')}</label>
										...
										<p>
											{ssoInfo?.isBound ? (
												<div className="flex items-center gap-2">
													<Badge color="green">{t('account_settings.sso_bound')}</Badge>
													{displayName} ID: {ssoInfo.uniqueId}
												</div>
											) : (
												<div className="flex items-center gap-2">
													<Badge color="gray">{t('account_settings.sso_unbound')}</Badge>
											...
											<Dialog.Root>
												...
												<Dialog.Content>
													...
													<Flex gap="2" justify="end" className="mt-4">
														<Dialog.Close>
															<Button variant="soft">{t('account_settings.cancel')}</Button>
														</Dialog.Close>
					...
					<Flex gap="4" align="center" justify="start">
						<label className="text-muted-foreground text-sm">{t('account_settings.looking_for_backup')}</label>
					</Flex>
				</Flex>
			</Flex>
		</Flex>
		...
		fetch(`/api/admin/2fa/enable?code=${encodeURIComponent(code)}`, {
			...
			.then(async res => {
				if (!res.ok) {
					...
					throw new Error(data.message || `Failed to enable 2FA (${res.status})`)
				}
				return res.json()
			})
			.then(() => {
				...
				<Dialog.Content>
					...
					<Flex direction="column" gap="2">
						...
						<div className="flex justify-center">
							{isLoading ? <Skeleton width="200px" height="200px" /> : <img src={qrcode!} alt="2FA QR Code" width={200} height={200} />}
						</div>
			...
			<div>
				<Dialog.Root open={isOpen} onOpenChange={setIsOpen}>
					...
					<Dialog.Content>
						...
						<Dialog.Description>{t('account.disable_2fa_confirmation')}</Dialog.Description>
						<Flex gap="2" justify="end" className="mt-4">
