...
import { SettingCardButton, SettingCardLabel, SettingCardLongTextInput, SettingCardSelect, SettingCardSwitch } from '@/components/admin/SettingCard'
		...
		fetch('/api/admin/settings/message-sender')
			...
			.then(data => {
				if (data.status === 'success' && data.data) {
					...
					const initialSender = settings.notification_method && senders.includes(settings.notification_method) ? settings.notification_method : ''
	...
	return (
		<>
			...
			<label className="text-muted-foreground text-sm flex flex-row items-center gap-1">
				...
				<Link to="/admin/notification/general">
