...
export default function GeneralSettings() {
	...
	const [geoip_testResult, setGeoipTestResult] = React.useState<string | null>(null)
	const [expected_usage, setExpectedUsage] = React.useState<string | null>(null)
	React.useEffect(() => {
		const pingPreserveTime = parseInt(settings.ping_record_preserve_time || '30', 10)
		const recordPreserveTime = parseInt(settings.record_preserve_time || '30', 10)
		if (isNaN(pingPreserveTime) || isNaN(recordPreserveTime)) {
			setExpectedUsage('0')
			return
		} else {
			setExpectedUsage(calculateExpectedUsage(pingPreserveTime, recordPreserveTime))
		}
	...
	return (
		<>
			...
			<SettingCardCollapse title={t('settings.geoip.test_title')} description={t('settings.geoip.test_description')}>
				<Flex className="w-full gap-2" direction="column">
					...
					<div>
						<Button
							variant="solid"
							onClick={async () => {
								const ip = (document.querySelector('input[placeholder]') as HTMLInputElement).value
								const result = await fetch(`/api/admin/test/geoip?ip=${ip}`)
								const data = await result.json()
								setGeoipTestResult(JSON.stringify(data.data, null, 2) || '无结果')
							}}>
					...
					<Flex className="w-full">
						{geoip_testResult && (
							<Code className="w-full whitespace-pre-wrap text-sm p-3 rounded-md overflow-auto max-h-96" style={{ display: 'block' }}>
								{geoip_testResult}
							</Code>
						)}
					</Flex>
			...
			<SettingCardMultiInputCollapse
				...
				onSave={async values => {
					...
					const pingPreserveTime = parseInt(values.ping_record_preserve_time, 10)
				...
				onChange={values => {
					...
					setExpectedUsage(calculateExpectedUsage(pingPreserveTime, preserveTime))
				}}>
			...
			<label className="text-sm text-muted-foreground -mt-4">{t('settings.nezha.description')}</label>
			<SettingCardSwitch
...
function calculateExpectedUsage(pingPreserveTime: number, recordPreserveTime: number): string {
