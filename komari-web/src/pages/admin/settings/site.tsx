...
export default function SiteSettings() {
	...
	return (
		<>
			...
			<label className="text-sm text-muted-foreground -mt-4">{t('settings.custom.note')}</label>
			<SettingCardLongTextInput
			...
			<SettingCardLongTextInput
				...
				description={t('settings.custom.body_description', '在页面底部添加自定义内容')}
			...
			<SettingCardCollapse
				...
				description={t('settings.custom.favicon_description', '在浏览器标签页显示的图标')}
				defaultOpen={true}>
				<Flex width={'100%'} justify="between" align="start" direction={'column'} gap="2">
					<Flex gap="2" align="center">
						{t('settings.custom.favicon_current', '当前 Favicon')}
						<img src="/favicon.ico" alt="Favicon" style={{ width: 32, height: 32 }} />
					</Flex>
					...
					<Flex gap="2" align="center">
						<Dialog.Root>
							<Dialog.Trigger>
								<Button color="tomato">{t('settings.custom.favicon_default', '恢复默认')}</Button>
							</Dialog.Trigger>
							<Dialog.Content>
								<Dialog.Title>{t('settings.custom.favicon_default', '恢复默认')}</Dialog.Title>
								<Dialog.Description>
									{t('settings.custom.favicon_default_description', '这将恢复默认的 Favicon 图标，是否继续？')}
								</Dialog.Description>
												...
												fetch('/api/admin/update/favicon', {
													...
													.then(data => {
														if (data.status === 'success') {
															toast.success(t('settings.custom.favicon_default_success', '已恢复默认 Favicon'))
														} else {
															toast.error(data.message || '恢复默认 Favicon 失败')
														}
													})
													.catch(error => {
														toast.error('' + error)
