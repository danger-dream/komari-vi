				...
				<Dialog.Root open={editOpen} onOpenChange={setEditOpen}>
					...
					<Dialog.Content>
						...
						<form onSubmit={handleEdit} className="flex flex-col gap-2">
							...
							<Select.Root value={form.metric} onValueChange={v => setForm(f => ({ ...f, metric: v as any }))}>
								<Select.Trigger />
								<Select.Content>
							...
							<TextField.Root
								...
								max="1"
								value={form.ratio}
								onChange={e => setForm(f => ({ ...f, ratio: Number(e.target.value) }))}
								required
							/>
...
const AddButton: React.FC = () => {
	...
	const { refresh } = useLoadAlert()
	const [selectedType, setSelectedType] = React.useState<'cpu' | 'ram' | 'disk' | 'net_in' | 'net_out'>('cpu')
			...
			<Dialog.Content>
				...
				<form onSubmit={handleSubmit}>
					<Flex direction="column" justify="end" gap="2" className="font-bold">
						...
						<Select.Root value={selectedType} onValueChange={value => setSelectedType(value as 'cpu' | 'ram' | 'disk' | 'net_in' | 'net_out')}>
						...
						<TextField.Root id="threshold" name="threshold" type="number" defaultValue={80} step="0.1" />
						<label htmlFor="ratio">{t('loadAlert.ratio')}</label>
						<TextField.Root id="ratio" name="ratio" type="number" step="0.1" min="0" max="1" defaultValue={0.8} />
						...
						<div className="flex items-center justify-start gap-2">
							...
							<label className="text-md font-normal">{t('common.selected', { count: selected.length })}</label>
						</div>
						<label htmlFor="interval">
							{t('ping.interval')} ({t('time.minute')})
						</label>
						<TextField.Root id="interval" name="interval" defaultValue={15} type="number" placeholder="15" />
