...
export default function AboutPage() {
	...
	const sortedLicenses = Object.entries(open_source_licenses).sort(([a], [b]) => a.localeCompare(b))
	...
	return (
		<div className="flex flex-col gap-4">
			...
			<SegmentedControl.Root defaultValue={view} onValueChange={setView}>
				<SegmentedControl.Item value="open_source">{t('about.open_source_title')}</SegmentedControl.Item>
				<SegmentedControl.Item value="eula">法律声明与合规指引</SegmentedControl.Item>
						...
						return (
							<>
								<div className="text-foreground flex flex-col gap-4">
									<SettingCardCollapse title="MIT License" description="Copyright (C) 2025 Komari Monitor">
										<pre className="text-wrap">{MIT_LICENSE}</pre>
									</SettingCardCollapse>
									<SettingCardCollapse title="Apache License" description="Version 2.0, January 2004">
										<pre className="text-wrap">{Apache2_LICENSE}</pre>
									</SettingCardCollapse>
								</div>
								<h2 className="text-xl font-semibold text-foreground">{t('about.open_source')}</h2>
								<div className="copyright text-sm text-gray-500 dark:text-gray-400">
									{sortedLicenses.map(([license, libs]) => (
										<div key={license} className="mb-2">
											<h3 className="font-black text-lg text-foreground">{license}</h3>
						...
						return (
							<>
								<div className="markdown-body border border-muted/20 rounded-md">
									{markdown ? <ReactMarkdown remarkPlugins={[remarkGfm]} children={markdown} /> : <Loading />}
								</div>
								<a
