...
import { Button, Card, Flex, TextField, Text, Separator, Badge } from '@radix-ui/themes'
		...
		pollingTimeoutRef.current = setTimeout(() => {
			...
			setResults(prevResults =>
				prevResults.map(result =>
					result.finished_at === null ? { ...result, finished_at: new Date().toISOString(), exit_code: -1, result: '执行超时' } : result
				)
			)
			clearPolling()
		...
		<div className="p-4 flex flex-col gap-3">
			...
			<Card className="p-6">
				<Flex direction="column" gap="4">
					<label className="text-xl font-bold">{t('exec.command')}</label>
					<TextField.Root value={command} onChange={e => setCommand(e.target.value)} placeholder={t('exec.commandPlaceholder')} size="3">
						<TextField.Slot>
							<Terminal size={16} />
						</TextField.Slot>
					</TextField.Root>
					...
					<Flex justify="end" gap="2">
						<Button onClick={executeCommand} disabled={executing || !command.trim() || selectedNodes.length === 0}>
							{executing ? (
								<>
									...
									<Card key={result.client} className="p-4">
										<Flex direction="column" gap="3">
											...
											<Flex justify="between" align="center">
												...
												{result.result && (
													<Button variant="ghost" size="1" onClick={() => copyOutput(result.result)}>
														<Copy size={14} />
													</Button>
												)}
											</Flex>
											...
											{/* 输出内容 */}
											{result.result && (
												<div className="bg-(--gray-2) rounded-md p-3 font-mono text-sm overflow-x-auto">
