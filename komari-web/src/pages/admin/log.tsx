...
const LogPage = () => {
	...
	React.useEffect(() => {
		const fetchLogs = async () => {
			setLoading(true)
			try {
				const response = await fetch(`/api/admin/logs?limit=${limit}&page=${page}`)
				if (!response.ok) {
	...
	return (
		<div className="flex flex-col gap-2 p-4">
						...
						{logs.map(log => (
							<TableRow key={log.id}>
								<TableCell>
									<Dialog.Root>
										<Dialog.Trigger>
											<label className="hover:underline font-bold">{log.id}</label>
										</Dialog.Trigger>
										<Dialog.Content>
											...
											<Flex direction="column" gap="1">
												...
												<label className="text-sm">{new Date(log.time).toLocaleString()}</label>
											</Flex>
											<Flex justify={'end'}>
												<Dialog.Close>
								...
								<TableCell>{log.message.length > 75 ? `${log.message.slice(0, 75)}...` : log.message}</TableCell>
			...
			<div className="flex justify-center items-center space-x-2 mt-4 gap-2">
				...
				{pageNumbers.map((p, i) =>
					typeof p === 'number' ? (
						<Button key={i} variant={p === page ? 'solid' : 'soft'} onClick={() => setPage(p)}>
							{p}
						</Button>
					) : (
