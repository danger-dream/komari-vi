								...
								<TableRow key={s.uuid}>
										...
										<Dialog.Root>
											<Dialog.Trigger>
												<label className="hover:underline cursor-pointer">
													{s.session.slice(0, 8)}...
													{isCurrent && <span className="ml-2 text-sm text-blue-600">{t('sessions.current')}</span>}
												</label>
											</Dialog.Trigger>
											<Dialog.Content>
												<Dialog.Title>{t('sessions.active_sessions')}</Dialog.Title>
												<Flex direction="column" gap="1">
													<label className="text-base font-bold">{t('sessions.session_id')}</label>
													<label className="text-sm">{s.session}</label>
													<label className="text-base font-bold">IP / {t('sessions.latest_ip', 'Latest IP')}</label>
													<label className="text-sm">
														{s.ip} / {s.latest_ip}
													</label>
													<label className="text-base font-bold">User Agent</label>
													<label className="text-sm">{s.user_agent}</label>
													<label className="text-sm text-muted-foreground font-bold">{UserAgentHelper.format(s.user_agent)}</label>
													<label className="text-base font-bold">{t('sessions.last_user_agent')}</label>
													<label className="text-sm">{s.latest_user_agent}</label>
													...
													<label className="text-base font-bold">{t('sessions.latest_online', 'Latest Online')}</label>
													<label className="text-sm">
													...
													<label className="text-sm">{new Date(s.created_at).toLocaleString()}</label>
													<label className="text-base font-bold">{t('sessions.expires_at', 'Expires At')}</label>
													<label className="text-sm">{new Date(s.expires).toLocaleString()}</label>
													<Flex justify={'end'}>
														<Dialog.Trigger>
									...
									<TableCell>
										<Dialog.Root>
											...
											<Dialog.Content>
												<Dialog.Title>{t('sessions.confirm_delete')}</Dialog.Title>
												<Dialog.Description>{t('sessions.delete_one_desc')}</Dialog.Description>
												<Flex gap="2" justify={'end'}>
													<Dialog.Trigger>
														<Button variant="soft">{t('sessions.cancel')}</Button>
													</Dialog.Trigger>
