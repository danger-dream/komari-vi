...
const PingChart = ({ uuid }: { uuid: string }) => {
	...
	if (typeof max_record_preserve_time === 'number' && max_record_preserve_time > 0) {
		...
		} else if (max_record_preserve_time > 1 && !presetViews.some(v => v.hours === max_record_preserve_time)) {
			const dynamicLabel =
				max_record_preserve_time % 24 === 0
					? `${t('chart.days', {
							count: Math.floor(max_record_preserve_time / 24)
	...
	const [hours, setHours] = useState<number>(avaliableView.find(v => v.label === initialView)?.hours || 1) // Add hours state
	...
	return (
		<Flex direction="column" align="center" gap="4" className="w-full max-w-screen">
			<div className="w-full overflow-x-auto px-2">
				<div className="w-max mx-auto">
					<SegmentedControl.Root
						...
						{avaliableView.map(v => (
							<SegmentedControl.Item key={v.label} value={v.label} className="capitalize">
								{v.label}
			...
			{latestValues.length > 0 ? (
					...
					<div
						...
						{latestValues.map(task => (
							<div key={task.id} className="flex flex-row items-center rounded">
								<div className="w-1 h-6 rounded-xs " style={{ backgroundColor: task.color }} />
				...
				<div className="w-full max-w-[900px] text-center text-muted-foreground mb-2">{t('common.none')}</div>
			)}
			<Card className="w-full max-w-[900px]">
				{chartData.length === 0 ? (
					<div className="w-full h-40 flex items-center justify-center text-muted-foreground">{t('common.none')}</div>
				) : (
					<ChartContainer config={chartConfig}>
						<LineChart data={chartData} accessibilityLayer margin={{ top: 0, right: 16, bottom: 0, left: 16 }}>
				...
				<div className="flex items-center justify-between gap-4" style={{ display: loading ? 'none' : 'flex' }}>
					...
					<Button variant="soft" size="2" onClick={toggleAllLines} className="flex items-center gap-2">
