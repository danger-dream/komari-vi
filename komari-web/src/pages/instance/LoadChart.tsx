...
const LoadChart = ({ data = [] }: LoadChartProps) => {
	...
	const avaliableView: { label: string; hours?: number }[] = [{ label: t('common.real_time') }]
	if (typeof max_record_preserve_time === 'number' && max_record_preserve_time > 0) {
		...
		} else if (max_record_preserve_time > 4 && !presetViews.some(v => v.hours === max_record_preserve_time)) {
			// 如果不是预设但大于4小时，显示具体小时
			const dynamicLabel =
				max_record_preserve_time % 24 === 0
					? `${t('chart.days', {
			...
			.then(resp => {
				...
				const mergedRecords = records.map((record: RecordFormat) => {
					...
					for (const deviceIndex in gpuDevices) {
						...
						const gpuRecord = device.records?.find((gr: any) => new Date(gr.time).getTime() === new Date(record.time).getTime())
				...
				// 按照时间升序排序
				mergedRecords.sort((a: RecordFormat, b: RecordFormat) => new Date(a.time).getTime() - new Date(b.time).getTime())
	...
	const realtimeData = Array.isArray(data) ? data.slice(-MAX_REALTIME_POINTS) : data
	...
	const chartData = isRealtime
		...
		: (() => {
				const selectedHours = presetViews.find(v => v.label === hoursView)?.hours || avaliableView.find(v => v.label === hoursView)?.hours || 24
	...
	return (
		<Flex direction="column" align="center" gap="4" className="w-full max-w-screen">
				...
				<div className="w-max mx-auto">
					<SegmentedControl.Root value={hoursView} onValueChange={setHoursView}>
						{avaliableView.map(view => (
							<SegmentedControl.Item key={view.label} value={view.label} className="capitalize">
			...
			<div
				...
				{/* CPU */}
				<Card className={cn}>
					{ChartTitle('CPU', live_data?.cpu?.usage ? `${live_data.cpu.usage.toFixed(2)}%` : '-')}
					<ChartContainer
						config={{
							cpu: {
								label: 'CPU',
						...
						<AreaChart data={chartData} accessibilityLayer margin={chartMargin}>
							...
							<XAxis dataKey="time" tickLine={false} tickFormatter={timeFormatter} interval={0} />
							<YAxis
								tickLine={false}
								axisLine={false}
								domain={[0, 100]}
							...
							<ChartTooltip
								...
								content={<ChartTooltipContent labelFormatter={lableFormatter} indicator="dot" />}
							/>
							<Area dataKey="cpu" animationDuration={0} stroke={primaryColor} fill={primaryColor} opacity={0.8} dot={false} />
						</AreaChart>
					</ChartContainer>
				</Card>
				{/* Ram */}
				<Card className={cn}>
					{ChartTitle(
						...
						<Flex gap="0" direction="column" align="end" className="text-sm">
							<label>{live_data?.ram?.used ? `${formatBytes(live_data.ram.used)} / ${formatBytes(node?.mem_total || 0)}` : '-'}</label>
							<label>{live_data?.swap?.used ? `${formatBytes(live_data.swap.used)} / ${formatBytes(node?.swap_total || 0)}` : '-'}</label>
						</Flex>
					)}
					<ChartContainer
						config={{
							...
							<ChartTooltip
								...
								formatter={(value, name, props) => {
									...
									if (typeof value === 'number') {
										...
										percent = typeof value[0] === 'number' ? value[0] : parseFloat(value[0] || '0')
									}
									return `${formatBytes(rawValue)} (${percent.toFixed(0)}%)`
								}}
								content={<ChartTooltipContent labelFormatter={lableFormatter} indicator="dot" />}
							/>
							<Area dataKey="ram" animationDuration={0} stroke={primaryColor} fill={primaryColor} opacity={0.8} dot={false} />
							<Area dataKey="swap" animationDuration={0} stroke={secondaryColor} fill={secondaryColor} opacity={0.8} dot={false} />
						</AreaChart>
					</ChartContainer>
				</Card>
				{/* Disk */}
				<Card className={cn}>
					{ChartTitle('Disk', live_data?.disk?.used ? `${formatBytes(live_data.disk.used)} / ${formatBytes(node?.disk_total || 0)}` : '-')}
					<ChartContainer
						config={{
							disk: {
								label: 'Disk',
						...
						<AreaChart data={chartData} accessibilityLayer margin={chartMargin}>
							...
							<YAxis
								...
								tickFormatter={(value, index) => (index !== 0 ? `${formatBytes(value)}` : '')}
								orientation="left"
								type="number"
								tick={{ dx: -10 }}
								mirror={true}
							...
							<ChartTooltip
								...
								content={<ChartTooltipContent labelFormatter={lableFormatter} indicator="dot" />}
							/>
							<Area dataKey="disk" animationDuration={0} stroke={primaryColor} fill={primaryColor} opacity={0.8} dot={false} />
						</AreaChart>
					</ChartContainer>
				</Card>
				{/* Netwodk */}
					...
					<ChartContainer
						...
						<LineChart data={chartData} accessibilityLayer margin={chartMargin}>
							...
							<ChartTooltip
								...
								content={<ChartTooltipContent labelFormatter={lableFormatter} indicator="dot" />}
							/>
							<Line dataKey="net_in" animationDuration={0} stroke={primaryColor} fill={primaryColor} opacity={0.8} dot={false} />
							<Line dataKey="net_out" animationDuration={0} stroke={colors[3]} fill={colors[3]} opacity={0.8} dot={false} />
						</LineChart>
					</ChartContainer>
				</Card>
				{/* Connections */}
					...
					<ChartContainer
						...
						<LineChart data={chartData} accessibilityLayer margin={chartMargin}>
							...
							<YAxis
								...
								mirror={true}
							/>
							<ChartTooltip cursor={false} content={<ChartTooltipContent labelFormatter={lableFormatter} indicator="dot" />} />
							<Line dataKey="connections" animationDuration={0} stroke={primaryColor} fill={primaryColor} opacity={0.8} dot={false} />
							<Line dataKey="connections_udp" animationDuration={0} stroke={colors[3]} fill={colors[3]} opacity={0.8} dot={false} />
						</LineChart>
					</ChartContainer>
				</Card>
				{/* Process */}
				<Card className={cn}>
					...
					<ChartContainer
						...
						<LineChart data={chartData} accessibilityLayer margin={chartMargin}>
							...
							/>
							<ChartTooltip cursor={false} content={<ChartTooltipContent labelFormatter={lableFormatter} indicator="dot" />} />
							<Line dataKey="process" animationDuration={0} stroke={primaryColor} fill={primaryColor} opacity={0.8} dot={false} />
						</LineChart>
					</ChartContainer>
				</Card>
							...
							<ChartContainer
								...
								<AreaChart
									data={chartData.map(item => ({
										...
										gpu_memory_raw:
											item.gpu_detailed?.[index]?.mem_used ?? (gpu.memory_total * (item.gpu_detailed?.[index]?.memory || 0)) / 100,
									...
									<ChartTooltip
										...
										content={<ChartTooltipContent labelFormatter={lableFormatter} indicator="dot" />}
									/>
									<Area dataKey="gpu_usage" animationDuration={0} stroke={primaryColor} fill={primaryColor} opacity={0.8} dot={false} />
									<Area dataKey="gpu_memory" animationDuration={0} stroke={secondaryColor} fill={secondaryColor} opacity={0.8} dot={false} />
									<Area dataKey="gpu_temp" animationDuration={0} stroke={colors[2]} fill={colors[2]} opacity={0.6} dot={false} />
								</AreaChart>
							</ChartContainer>
						</Card>
					))}
