...
import { Button, Card, Code, Dialog, Flex, IconButton, TextArea, TextField } from '@radix-ui/themes'
	...
	const InnerLayout = () => {
		...
		return (
			<Flex
				...
				<Flex>
					<label className="text-lg font-semibold">{t('command_clipboard.title')}</label>
				</Flex>
			...
			<Dialog.Content>
				...
				<form onSubmit={handleAddCommand}>
					<Flex direction="column" gap="2">
						...
						<TextField.Root id="name" name="name" defaultValue={Math.random().toString(36).substring(7)} />
						...
						<TextField.Root defaultValue={0} type="number" id="weight" name="weight" />
...
const EditButton = ({ id, name, text, remark, weight }: CommandClipboard) => {
	...
	const handleUpdate = async (event: React.FormEvent) => {
		...
		try {
			setUpdating(true)
			await updateCommand(id, newName, newText, newRemark, weight ? parseInt(weight) : 0)
			setOpen(false)
			...
			<Dialog.Content>
				...
				<form onSubmit={handleUpdate}>
					<Flex direction="column" gap="2">
						...
						<label htmlFor={`weight`}>{t('common.weight')}</label>
						<TextField.Root type="number" id={`weight`} name="weight" defaultValue={weight} />
						<Flex justify="end" gap="2" className="mt-4">
							<Dialog.Close>
								<Button variant="soft">{t('common.cancel')}</Button>
							</Dialog.Close>
			...
			<Card>
				<Flex direction="column" gap="2">
					<Flex justify="between" align="center">
						...
						<Button onClick={() => sendCommand(item.text)}>{t('common.execute')}</Button>
					</Flex>
