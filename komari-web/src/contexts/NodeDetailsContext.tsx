...
export const NodeDetailsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
		...
		fetch('/api/admin/client/list')
			...
			.then((data: NodeDetail[]) => {
				setNodeDetail(
					(data || []).map(n => ({
						...n,
						billing_cycle: n.billing_cycle === 0 ? 30 : n.billing_cycle
					}))
				)
				setIsLoading(false)
			})
	...
	return <NodeDetailsContext.Provider value={{ nodeDetail, isLoading, error, refresh }}>{children}</NodeDetailsContext.Provider>
}
