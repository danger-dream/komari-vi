...
export const NodeListProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
		...
		call<{ uuid?: string }, Record<string, any>>('common:getNodes')
			.then(result => {
				...
				const list: NodeBasicInfo[] = Object.values(result).map((n: any) => ({
					...
					price: n.price ?? 0,
					tags: n.tags ?? '',
					billing_cycle: (n.billing_cycle ?? 0) === 0 ? 30 : (n.billing_cycle ?? 0),
	...
	return <NodeListContext.Provider value={{ nodeList, isLoading, error, refresh }}>{children}</NodeListContext.Provider>
}
