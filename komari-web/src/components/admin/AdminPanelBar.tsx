...
const AdminPanelBar = ({ content }: AdminPanelBarProps) => {
	...
	useEffect(() => {
		...
		async function loadReleases() {
			/* try {
				const resp = await fetch('https://api.github.com/repos/komari-monitor/komari/releases?per_page=100', {
					headers: {
				...
				if (ignore) return
				const valid = (data || []).filter(r => !r.draft && !r.prerelease).filter(r => isNewerVersion(r?.tag_name || r?.name, currentVersion))
	...
	useEffect(() => {
		...
		combined.forEach(item => {
			if (item.children) {
				newState[item.path] = item.children.some((child: MenuItem) => location.pathname === child.path || location.pathname.startsWith(child.path))
			}
		})
	...
	return (
		<>
			<Grid
				...
				{/* Navbar */}
				<motion.nav className="col-span-2" initial={{ y: 0 }} animate={{ y: 0 }} transition={{ duration: 0.5, ease: 'easeOut' }}>
					<Flex gap="3" p="2" justify="between" align="center" className="border-b-1">
						<div className="flex items-center gap-2">
							...
							{updateAvailable && releasesSince.length > 0 && (
								<Tips mode="dialog" className="check-update" trigger={<CircleFadingArrowUp color="#FB4141" size="16" />}>
									<div className="flex flex-col gap-2 max-w-[80vw] md:max-w-[720px]">
										...
										<div className="text-sm text-muted-foreground flex">
											<span style={{ marginRight: 8 }}>{(publicInfo as any)?.version || versionInfo?.version}</span>
											<span>{'> '}</span>
							...
							<label className="text-sm text-muted-foreground self-end overflow-hidden" hidden={isMobile}>
								{(publicInfo as any)?.version || (versionInfo && `${versionInfo.version} (${versionInfo.hash})`)}
							</label>
						</div>
				...
				<AnimatePresence>
					<motion.div
						...
						<Flex gap="3" className="p-2 border-r-1" direction="column" justify="start" align="start" style={{ height: '100%', minWidth: '240px' }}>
							{/* 关闭按钮 */}
							<IconButton
								variant="soft"
								style={{
							...
							{/* 侧边连链接 */}
							<Flex direction="column" gap="1" className="h-full md:mt-0 mt-6" style={{ width: '100%' }}>
								{[...baseMenuItems, ...extraMenuItems].map((item: ExtendedMenuItem) => {
									// 支持 icon 为 URL/相对路径
									const isOpen = openSubMenus[item.path]
									const renderIcon = (icon: string, labelKey: string, className?: string, active?: boolean) => {
										...
										return (
											<div key={item.path}>
												<Flex
													...
													{renderIcon(item.icon, item.labelKey, 'flex w-4 h-5 items-center justify-center')}
													<Text
														className="text-base"
														weight="medium"
														style={{
													...
													<Flex direction="column" className="ml-4 gap-1">
														{item.children.map((child: MenuItem) => (
															<SidebarItem
																key={child.path}
																to={child.path}
																icon={renderIcon(child.icon, child.labelKey, 'flex w-4 h-5 items-center justify-center')}
									...
									return (
										<SidebarItem
											key={item.path}
											to={item.path}
											icon={renderIcon(item.icon, item.labelKey, 'flex w-4 h-5 items-center justify-center')}
...
// 侧边栏项目组件
const SidebarItem = ({ to, onClick, icon, children, newTab }: { to: string; onClick: () => void; icon: ReactNode; children: ReactNode; newTab?: boolean }) => {
	const location = useLocation()
	const isExternalLink = to.startsWith('http://') || to.startsWith('https://')
	const isActive = !isExternalLink && to !== '/' && (location.pathname === to || (to !== '/admin' && location.pathname.startsWith(to)))
	...
	return (
		<Link to={to} onClick={onClick} className="group transition-colors duration-200 hover:bg-accent-3 rounded-md">
			<Flex
				className="p-2 gap-2"
				align="center"
				style={{
