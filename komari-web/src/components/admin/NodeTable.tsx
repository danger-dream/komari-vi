...
const columns: ColumnDef<z.infer<typeof schema>>[] = [
	...
	},
	{
		...
		cell: ({ row }) => (
			<div className="flex items-center justify-center">
				<Checkbox size={'1'} checked={row.getIsSelected()} onCheckedChange={value => row.toggleSelected(!!value)} />
			</div>
		),
		enableSorting: false,
		enableHiding: false
...
export function DataTable() {
	...
	const dataIds = React.useMemo<UniqueIdentifier[]>(() => data?.map(({ uuid }) => uuid) || [], [data])
	...
	return (
        ...
        ${!isMobile ? 'p-4' : ''}
      `}>
			<h2 className="text-2xl font-bold mb-4">{t('admin.nodeTable.nodeList')}</h2>
				...
				<Dialog.Root>
					<Dialog.Trigger>
						<Button>
							...
							<span className="hidden lg:inline">{t('admin.nodeTable.addNode')}</span>
						</Button>
					</Dialog.Trigger>
					<Dialog.Content>
						<Dialog.Title>{t('admin.nodeTable.addNode')}</Dialog.Title>
						<div className="">
							<label className="block mb-1 text-sm font-medium text-muted-foreground">{t('admin.nodeTable.nameOptional')}</label>
							<TextField.Root
										...
										{table.getRowModel().rows?.length ? (
											...
											<TableRow>
												<TableCell colSpan={columns.length} className="h-24 text-center">
													{data.length === 0 && !isLoading ? t('admin.nodeTable.noData') : t('admin.nodeTable.noResults')}
												</TableCell>
											</TableRow>
										)}
									</TableBody>
