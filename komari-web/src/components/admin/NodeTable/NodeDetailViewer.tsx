		...
		<Drawer direction={isMobile ? 'bottom' : 'right'}>
			<DrawerTrigger asChild>
				<div className="h-8 flex items-center">
					<label className="text-foreground w-fit text-md px-0 text-left hover:underline font-bold">{item.name}</label>
				</div>
			</DrawerTrigger>
			<DrawerContent>
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
							<div className="flex flex-col gap-3">
								...
								<span id="detail-createdAt" className="bg-muted px-3 py-2 rounded border select-text">
									{item.created_at ? new Date(item.created_at).toLocaleString() : <span className="text-muted-foreground">-</span>}
								</span>
							</div>
							<div className="flex flex-col gap-3">
								...
								<span id="detail-updatedAt" className="bg-muted px-3 py-2 rounded border select-text">
									{item.updated_at ? new Date(item.updated_at).toLocaleString() : <span className="text-muted-foreground">-</span>}
								</span>
							</div>
						</div>
					</form>
