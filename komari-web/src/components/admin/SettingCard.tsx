import { Button, DropdownMenu, Flex, IconButton, Switch, TextArea, TextField } from '@radix-ui/themes'
...
export function SettingCard({
	...
	const actionChild = React.Children.toArray(children).find(child => React.isValidElement(child) && child.type === Action)

	const otherChildren = React.Children.toArray(children).filter(child => !(React.isValidElement(child) && child.type === Action))
	...
	return (
		<Flex
			...
			className={bordless ? 'border-0' : 'border-1 rounded-md py-2 px-4 bg-transparent  min-h-8' + className}>
			<Flex className="w-full" direction="row" justify="between" align="center" wrap="nowrap" onClick={onHeaderClick}>
...
export function SettingCardSwitch({
	...
	const handleChange = (c: boolean) => {
		...
		setChecked(c)
		const result: any = onChange && switchRef.current ? onChange(c, switchRef.current) : undefined
		if (autoDisabled) {
		...
		<SettingCard {...props} direction="column">
			<SettingCard.Action>
				<Flex direction="row" gap="2" align="center">
					<label>{label}</label>
					<Switch ref={switchRef} checked={checked} onCheckedChange={handleChange} disabled={disabled} />
				</Flex>
			</SettingCard.Action>
		</SettingCard>
	)
...
interface SettingCardShortTextInputProps extends Omit<React.ComponentProps<typeof TextField.Root>, 'onChange' | 'onKeyDown'> {
	// SettingCard 相关属性
	title?: string
	description?: string
	bordless?: boolean
	...
	// 保存回调
	OnSave?: (value: string, inputElement: HTMLInputElement, buttonElement: HTMLButtonElement) => void
...
export function SettingCardShortTextInput({
	...
	const handleSave = () => {
		...
		const result: any = inputRef.current && buttonRef.current ? OnSave(valueToSave, inputRef.current, buttonRef.current) : undefined
		if (autoDisabled) {
			const promise: Promise<any> = result
			if (promise && typeof promise.then === 'function') {
		...
		<SettingCard title={title} description={description} bordless={bordless}>
			<Flex direction="column" className="w-full mt-1" gap="2" align="start">
				<TextField.Root
					...
					ref={inputRef}></TextField.Root>
				{children}
				<Button ref={buttonRef} onClick={handleSave} variant="solid" hidden={!showSaveButton} disabled={savingState}>
					{label}
				</Button>
			</Flex>
		</SettingCard>
...
export function SettingCardLongTextInput({
	...
	label?: string
	defaultValue?: string
	OnSave?: (value: string, textAreaElement: HTMLTextAreaElement, buttonElement: HTMLButtonElement) => void
	...
	const handleSave = () => {
		...
		const result: any = textAreaRef.current && buttonRef.current ? OnSave(value, textAreaRef.current, buttonRef.current) : undefined
		if (autoDisabled) {
			const promise: Promise<any> = result
			if (promise && typeof promise.then === 'function') {
	...
	return (
		<SettingCard title={title} description={description} bordless={bordless}>
			<Flex direction="column" className="w-full mt-1" gap="2" align="start">
				<TextArea className="w-full" defaultValue={defaultValue} resize="vertical" value={value} onChange={handleTextAreaChange} ref={textAreaRef} />
				{showSaveButton && (
					<Button ref={buttonRef} onClick={handleSave} variant="solid" disabled={savingState}>
						{label}
					</Button>
				)}
			</Flex>
...
export function SettingCardSelect({
	...
	const [disabled, setDisabled] = React.useState(false)
	const savingState = isSaving !== undefined ? isSaving : disabled
	const [selectedValue, setSelectedValue] = React.useState(value !== undefined ? value : defaultValue)
	const buttonRef = React.useRef<HTMLButtonElement>(null)
...
export function SettingCardLabel({ children }: { children: React.ReactNode | null }) {
	return (
...
export function SettingCardCollapse({
	...
	return (
		<SettingCard title={title} description={description} onHeaderClick={() => setOpen(!open)} bordless={bordless}>
			<SettingCard.Action>
				<IconButton variant="soft" onClick={() => setOpen(!open)} aria-expanded={open} aria-controls="collapsible-content">
					<motion.div
...
SettingCardCollapse.Header = function Header({ children }: { children: React.ReactNode }) {
