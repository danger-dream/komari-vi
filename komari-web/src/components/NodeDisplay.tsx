...
import { Flex, Text, IconButton, TextField, SegmentedControl } from '@radix-ui/themes'
	...
	return (
		<div className="w-full">
			...
			<Flex
				...
				{/* 搜索框 */}
				<Flex align="center" gap="2" className="flex-1 max-w-md relative" wrap="wrap">
					<TextField.Root
						ref={searchRef}
				...
				<Flex align="center" gap="2">
					<label className="whitespace-nowrap text-md text-muted-foreground">{t('view.mode', { defaultValue: '显示模式' })}</label>
					<Flex gap="1">
						<IconButton
			...
			{showGroupSelector && (
				<Flex align="center" gap="2" className="mx-4 mb-2 -mt-2 overflow-x-auto">
					<label className="whitespace-nowrap text-md text-muted-foreground">{t('common.group', { defaultValue: '分组' })}</label>
					<SegmentedControl.Root value={selectedGroup} onValueChange={setSelectedGroup} size="1">
						<SegmentedControl.Item value="all">{t('common.all', { defaultValue: '所有' })}</SegmentedControl.Item>
						{groups.map(group => (
					...
					<Text size="2" color="gray">
						{t('search.results', {
							...
							defaultValue: `找到 ${filteredNodes.length} 个服务器，共 ${
								selectedGroup === 'all' ? nodes.length : nodes.filter(n => n.group === selectedGroup).length
							} 个`
						})}
					</Text>
				) : (
					<Text size="2" color="gray">
						{selectedGroup === 'all'
							? t('nodeCard.totalNodes', {
									...
									defaultValue: `共 ${nodes.length} 个节点，${liveData?.online?.length || 0} 个在线`
							  })
							: t('nodeCard.groupNodes', {
									group: selectedGroup,
									total: filteredNodes.length,
									online: filteredNodes.filter(n => liveData?.online?.includes(n.uuid)).length,
									defaultValue: `${selectedGroup} 分组：共 ${filteredNodes.length} 个节点，${
			...
			{filteredNodes.length === 0 ? (
				<Flex direction="column" align="center" justify="center" className="py-16 mx-4">
					<Text size="4" color="gray" className="mb-2">
						{searchTerm.trim() ? t('search.no_results', { defaultValue: '未找到匹配的节点' }) : t('nodes.empty', { defaultValue: '暂无节点数据' })}
					</Text>
					{searchTerm.trim() && (
						<Text size="2" color="gray">
