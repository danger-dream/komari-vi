...
const MiniPingChart = ({ uuid, width = '100%', height = 300, hours = 12 }: MiniPingChartProps) => {
	...
	const chartData = useMemo(() => {
		...
		const validIntervals = tasks.map(t => t.interval).filter((v): v is number => typeof v === 'number' && v > 0)
		...
		let rows = Object.values(grouped).sort((a: any, b: any) => new Date(a.time).getTime() - new Date(b.time).getTime())
	...
	return (
		<Card style={{ width, height }} className="flex flex-col">
			...
			{!loading && !error && chartData.length === 0 ? (
				<div className="w-full h-full flex items-center justify-center text-muted-foreground">{t('common.none')}</div>
			) : (
				!loading &&
				!error && (
					<ChartContainer config={chartConfig} className="w-full h-full">
						<LineChart data={chartData} accessibilityLayer margin={{ top: 10, right: 16, bottom: 10, left: 16 }}>
