...
} & Omit<React.ComponentProps<'input'>, 'value' | 'defaultValue' | 'onChange' | 'placeholder' | 'disabled'>
 ...
 */
export function SelectOrInput<T extends Primitive = string>(props: SelectOrInputProps<T>) {
	const {
		options,
		value,
		defaultValue,
	...
	const normalizedOptions = React.useMemo<SelectOption<T>[]>(() => {
		return options.map(
			(opt): SelectOption<T> => (typeof opt === 'string' || typeof opt === 'number' ? { label: String(opt), value: opt as T } : (opt as SelectOption<T>))
		)
	}, [options])
	...
	const handleKeyDown: React.KeyboardEventHandler<HTMLInputElement> = e => {
		...
		if (e.key === 'ArrowDown') {
			e.preventDefault()
			setHighlightIndex(i => Math.min((i < 0 ? -1 : i) + 1, Math.max(displayed.length - 1, 0)))
			...
			{open && (
				<div
					...
					<ul id={listId} ref={listRef} role="listbox" className="p-1">
						{displayed.length === 0 ? (
							allowCustom ? null : (
								<li className={cn('text-muted-foreground select-none rounded-sm px-2 py-1.5 text-sm')}>{emptyText}</li>
							)
						) : (
								...
								return (
									<li
										...
										className={cn(
											...
											isActive ? 'bg-accent-10 text-accent-foreground' : 'hover:bg-accent hover:text-accent-foreground',
											optionClassName
										)}
