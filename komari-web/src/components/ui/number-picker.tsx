...
export default function NumberPicker({ defaultValue, onChange, min = 1, max = 100, ...props }: NumberPickerProps) {
	...
	return (
		<Flex align="center" gap="2">
			<IconButton variant="soft" radius="full" size="1" onClick={() => handleChange(currentValue - 1)} disabled={isMinDisabled} aria-label="decrement">
				<Minus size="16" />
			</IconButton>
			<TextField.Root
				type="text"
			...
			<IconButton variant="soft" radius="full" size="1" onClick={() => handleChange(currentValue + 1)} disabled={isMaxDisabled} aria-label="increment">
				<Plus size="16" />
			</IconButton>
		</Flex>
	)
