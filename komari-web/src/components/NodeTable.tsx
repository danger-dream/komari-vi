...
const NodeTable: React.FC<NodeTableProps> = ({ nodes, liveData }) => {
		...
		return (event: React.MouseEvent) => {
			...
			setSortState(prev => {
				if (prev.field === field) {
					...
					const nextOrder: SortOrder = prev.order === 'default' ? 'asc' : prev.order === 'asc' ? 'desc' : 'default'
	...
	return (
		<div className="mx-4 overflow-x-auto node-table-container">
			<Table>
				<TableHeader>
					<TableRow>
						...
						<TableHead className="cursor-pointer hover:bg-accent-2 select-none" onClick={handleSort('os')} title={t('nodeCard.sortTooltip')}>
							<Flex align="center" gap="1">
								{t('nodeCard.os')}
								{getSortIcon('os')}
							</Flex>
						...
						<TableHead className="cursor-pointer hover:bg-accent-2 select-none" onClick={handleSort('cpu')} title={t('nodeCard.sortTooltip')}>
							<Flex align="center" gap="1">
								{t('nodeCard.cpu')}
								{getSortIcon('cpu')}
							</Flex>
						</TableHead>
						<TableHead className="cursor-pointer hover:bg-accent-2 select-none" onClick={handleSort('ram')} title={t('nodeCard.sortTooltip')}>
							<Flex align="center" gap="1">
								{t('nodeCard.ram')}
								{getSortIcon('ram')}
							</Flex>
						</TableHead>
						<TableHead className="cursor-pointer hover:bg-accent-2 select-none" onClick={handleSort('disk')} title={t('nodeCard.sortTooltip')}>
							<Flex align="center" gap="1">
								{t('nodeCard.disk')}
								{getSortIcon('disk')}
							</Flex>
						</TableHead>
						<TableHead className="cursor-pointer hover:bg-accent-2 select-none" onClick={handleSort('price')} title={t('nodeCard.sortTooltip')}>
							<Flex align="center" gap="1">
								{t('nodeCard.price')}
								{getSortIcon('price')}
							</Flex>
								...
								<TableRow
									...
									<TableCell className="node-name-cell">
										<Flex align="center" gap="1">
											...
											<Link to={`/instance/${node.uuid}`} className="hover:underline" onClick={e => e.stopPropagation()}>
												<Flex direction="column" gap="0">
													<label className="max-w-[150px] font-bold text-lg truncate">{node.name}</label>
													{isOnline ? (
														<label className="-mt-1 text-muted-foreground text-xs">{formatUptime(nodeData.uptime, t)}</label>
													) : (
														<label className="-mt-1 text-muted-foreground">-</label>
													)}
												</Flex>
											</Link>
										</Flex>
									...
									<TableCell>
										<Flex direction="row" justify="start" align="center" gap="1">
											...
											{nodeData.message && <Tips color="#CE282E">{nodeData.message}</Tips>}
										</Flex>
									</TableCell>
