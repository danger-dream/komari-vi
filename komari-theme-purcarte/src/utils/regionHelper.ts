...
/**
 * 获取地域优先级
 * @param region 地区代码或emoji
 * @returns 优先级数字，数字越小优先级越高
 */
export const getRegionPriority = (region: string): number => {
	return REGION_PRIORITY[region] || REGION_PRIORITY[region.toUpperCase()] || 999
}
 ...
 * @returns 排序结果
 */
export const sortByRegionPriority = (a: string, b: string): number => {
	const priorityA = getRegionPriority(a)
	const priorityB = getRegionPriority(b)
	if (priorityA !== priorityB) {
		return priorityA - priorityB
	}
	// 优先级相同时按名称排序
	return getRegionDisplayName(a).localeCompare(getRegionDisplayName(b), 'zh-CN')
}
